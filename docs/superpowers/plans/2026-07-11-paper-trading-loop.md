...
- Mapping (binding, spec §3.2): StrongBuy/Buy no-position → `market_buy` sized `position_size_pct` (default env DEFAULT_SIZE_PCT) percent of current equity, stop = `stop_loss` or entry×(1−DEFAULT_STOP_PCT/100), tp = `take_profit` or `price_target` (as single TP, fraction 1.0). Buy with existing position → add up to symbol cap. Hold → `set_risk` with any provided stop/TP; nothing else. Sell → `market_sell` fraction 0.5; StrongSell → fraction 1.0; either with no position → ledger noop entry (`order_type="noop"`, note="sell signal with no position"). Rating read case-insensitively (journal stores "Hold").
